...
description: The four-wheel differential-drive base under ROS 2 Jazzy — packages, bring-up, topics, and the systemd service.
---
...
4. `haller_vision` pipeline — IMX219 camera + detection + segmentation + traversability (skip with `enable_vision:=false`).
...
- **Vision pipeline internals** — `haller_vision` (camera, detection, segmentation, traversability) has its own pipeline launch and config. A dedicated page is TODO.
