...
description: Rent a cloud GPU, smoke-test π0.5 / pi0 / GR00T on an SO-101 dataset, replay-eval against your recordings, and LoRA-finetune to your task.
---
...
<Callout type="info">
Companion guide: [Dataset collection](/docs/data/dataset-collection) — how to record the SO-101 dataset you'll evaluate against. All the scripts referenced below live in [`scripts/runpod/`](https://github.com/oscardvs/haller_ws/tree/main/scripts/runpod).
</Callout>
...
<Callout type="info">
You can also bake the token into the pod via the RunPod "Environment variables" UI as `HF_TOKEN=...`; the `hf` CLI picks it up automatically and you skip the interactive login.
</Callout>
...
Once you've recorded a dataset (see [Dataset collection](/docs/data/dataset-collection)) and pushed it to the Hub, evaluate it:
...
- **Closed-loop deployment** (live policy → real arm over the network). Needs a websocket policy server, a local bridge owning the USB + cameras, and Tailscale to keep latency predictable. See the Phase 3 roadmap entry in [Dataset collection](/docs/data/dataset-collection#roadmap).
