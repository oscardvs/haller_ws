...
Hilti B+
  └─ disconnect ≥30 A
       └─ 15 A main fuse
            ├─ 10 A ─ [adj. buck 8-40 V → 7.4 V, 10 A, CC+CV]
            │           └─ 10 A fast-blow ─ TVS 8.0 V crowbar ─ E-STOP relay
            │                └─ WAGO "7V4" node
            │                     ├─ 6 A ─ Waveshare R ─ arm R bus
            │                     └─ 6 A ─ Waveshare L ─ arm L bus
            └─  5 A ─ [sealed buck 15-40 V → 12 V, 10 A]
                          ├─ Jetson Orin Nano (5.5 × 2.5 mm barrel)
                          └─ powered USB hub (12 V DC in)

Hilti B− ─ star ground (single WAGO node), 14 AWG ─ all converter and board returns
Nuron data wires (white + blue) ─ individually insulated, not connected
```
...
| 12 V | USB hub + wrist cams | ~1 A | 12 W |
| **Pack input** | at 21.6 V, 90% efficiency | **~6.7 A** | **~146 W** |

The 15 A main fuse covers this with wide headroom. Runtime at a realistic ~60 W
