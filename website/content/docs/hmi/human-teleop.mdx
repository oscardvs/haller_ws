...
```
┌─────────────────────────────────────┐
│  Browser (/teleop/human)            │
│  ┌─ getUserMedia → <video>          │
│  ├─ MediaPipe Pose + Hands  (local) │
│  ├─ KeypointFrame { left, right,    │
│  │                  pinch_calib,    │
│  │                  dead_man, ts }  │
│  └─ HumanTeleopClient (WS sender)   │
└──────────────┬──────────────────────┘
               │  WS /ws/teleop/human/in
               │  one frame per render tick
┌──────────────▼──────────────────────┐
│  Backend (FastAPI)                  │
│  ┌─ ingest_frame                    │
│  ├─ retarget.compute_joint_goal     │
│  │     shoulder, elbow, wrist…      │
│  │     pinch → gripper aperture     │
│  ├─ rate cap (4°/tick) + clamp      │
│  └─ ArmManager.write_goal @ 60 Hz   │
└──────────────┬──────────────────────┘
               │  Feetech bus
┌──────────────▼──────────────────────┐
