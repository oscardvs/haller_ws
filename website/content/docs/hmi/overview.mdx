...
The backend reads `hmi/backend/config.yaml` by default. To run with a different config — most commonly one of the [MuJoCo sim presets](/docs/sim/mujoco) — pass `--config` to `run_hmi.sh`:
...
- **Teleop card** — leader↔follower mirroring between the two arms at 60 Hz. See [Leader ↔ follower teleop](/docs/hmi/teleop).
- **Human teleop link** — opens `/teleop/human` for webcam-driven bimanual control. See [Human-pose teleop](/docs/hmi/human-teleop).
...
Common HMI runtime issues (live badge red, 409 on goal, calibration banner stuck, joint sliders frozen) are listed on the [Troubleshooting](/docs/troubleshooting#hmi-is-up-but-broken) page.
