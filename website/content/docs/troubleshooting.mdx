...
The leader wasn't being driven (you started recording without moving it), or the follower was stuck in `STOP` mode. Use `lerobot-dataset-viz --repo-id <ds> --episode-index=0` to verify the joint state/action arrays before training. (In lerobot 0.5.x this command is `lerobot-dataset-viz`; older docs call it `lerobot-visualize-dataset`, which no longer exists.)
...
Try in order: drop batch size (`BATCH_SIZE=8 scripts/runpod/finetune_pi05.sh ...`); confirm `--policy.gradient_checkpointing=true` is set; move to a larger GPU (A100 40 GB).
