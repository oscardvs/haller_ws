...
A Meta Quest drives one or both SO-101 arms from the headset browser. It is
the only teleop input path: there is no webcam mode and no keyboard clutch. The
MediaPipe pipeline, the mouth clutch and the body-angle modes were deleted in
the 2026-08-22 unification.

One session has two surfaces:
...
- **`/teleop/vr`** is that URL: the WebXR page, passthrough AR, with the
  in-headset HUD (two world-locked quads and a grabbable cluster).
...
| GET | `/config` | none | arms, cameras, version |
...
| GET  | `/arm/{id}/presets` | none | `{names: [...]}` saved poses for this arm |
| DEL  | `/arm/{id}/preset/{name}` | none | delete a saved pose |
| GET  | `/teleop` | none | current teleop status |
...
| GET | `/teleop/human` | none | headset-session status: per-side authority, clutch, `goal_deg`, collision |
...
| GET  | `/calibration/status` | none | per-arm calibration file status; current session if active |
| POST | `/calibration/{arm_id}/start` | none | begin a calibration session; 409 if any arm isn't manual |
| POST | `/calibration/{arm_id}/capture_neutral` | none | capture current pose as the new 0°; transitions to sweep |
| POST | `/calibration/{arm_id}/finish_sweep` | none | end the range-of-motion sweep; 422 if any joint unmoved |
| POST | `/calibration/{arm_id}/save` | none | write the new calibration (with backup) and reload the arm |
| POST | `/calibration/{arm_id}/abort` | none | cancel the session (idempotent); restores torque |
| GET  | `/cameras` | none | configured cameras + runtime `active` and `record` flags |
| POST | `/cameras/{id}/record` | `{record}` | move a camera in/out of the recorded set; 409 while an episode is open |
| GET  | `/cameras/{id}/snapshot` | none | single JPEG (503 if placeholder or disconnected) |
| GET  | `/cameras/{id}/stream` | none | `multipart/x-mixed-replace` MJPEG live feed (~15 Hz to the browser) |
| GET  | `/record/status` | none | recorder status: repo, task, frame counts, last error |
...
| GET  | `/record/repos` | none | `{root, repos: [{repo_id, episodes, frames, size_bytes}]}`; scan of the lerobot home |
...
| WS | `/ws/telemetry` | none | ~20 Hz frames: base + arms state + teleop + `human_teleop` + alerts |
| WS | `/ws/teleop/vr/in` | none | the one teleop socket. In: `vr_keypoints` / `xr_frame`, `config_update`, `request_settings`. Out: `ik_state` (20 Hz while frames flow), `config_applied`, `settings` |
...
- **The session is running, the grips are squeezed, and nothing moves.** Read the side's chip. `ACQUIRING` means the countdown and rate ramp are still serving; hold the pose. `NOT IN SESSION` means you started a solo preset and this is the side it ignores. A `HOLD` chip reading `tracking lost` means that controller stopped reporting.
