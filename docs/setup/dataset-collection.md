...
`observation.wall_clock` is the **real** capture time of the frame: LeRobot's
own `timestamp` is synthetic (`frame_index / fps`), so a skipped tick leaves no
gap there, and this channel is the only way to see real sampling holes after the
fact.
